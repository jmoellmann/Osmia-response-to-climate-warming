---
title: "Sex determination alysis"
...
setwd("~/Documents/PhD/analyses/sex_determination/")

weights_wide <- read_tsv(file = "../cocoon_weight_analysis/weights_furbished.tsv") %>% 
   dplyr::rename(population = pop) %>% arrange(species, population) %>% 
   mutate(relID = paste0(group, "-", ID)) %>% 
   dplyr::select(-ID, -group) %>% mutate(ID = 1:nrow(.))
sizes <- read_tsv(file = "../cocoon_size_analysis/results/size.tsv") %>% 
...
### ---> Sometimes optimal K is around 2 (the sexes?), sometimes it is around 6 (not sure what is driving this)
...
```{r}
ggsave("results/clustering_with_weight.png", km_plot, width = 12, height = 7)
```
...
```{r}
write_tsv(df_km, "results/data_km.tsv")
write_tsv(df, "results/data.tsv")
```
...
```{r}
full_df <- read_tsv("../weight_loss_tracking/full_df.tsv")
```
